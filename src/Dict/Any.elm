module Dict.Any exposing
    ...
    , any, anyEquals, find, findMap, first, isSingleton, last
    )

{-| A dictionary mapping unique keys to values.
Insert, remove, and query operations all take _O(log n)_ time.
Derived from elm/core@1.0.5 Dict module.
...
-}
get : (k -> comparable) -> k -> Dict k v -> Maybe v
get toComparable targetKey dict =
    case dict of
        RBEmpty_elm_builtin ->
            Nothing

        RBNode_elm_builtin _ key value left right ->
            case compare (toComparable targetKey) (toComparable key) of
                LT ->
                    get toComparable targetKey left

                EQ ->
                    Just value

                GT ->
                    get toComparable targetKey right


{-| Determine if a key is in a dictionary.
-}
member : (k -> comparable) -> k -> Dict k v -> Bool
member toComparable key dict =
    case get toComparable key dict of
        Just _ ->
            True
...
a collision.
-}
insert : (k -> comparable) -> k -> v -> Dict k v -> Dict k v
insert toComparable key value dict =
    -- Root node is always Black
    case insertHelp toComparable key value dict of
...
insertHelp : (k -> comparable) -> k -> v -> Dict k v -> Dict k v
insertHelp toComparable key value dict =
    case dict of
        RBEmpty_elm_builtin ->
        ...
        RBNode_elm_builtin nColor nKey nValue nLeft nRight ->
            case compare (toComparable key) (toComparable nKey) of
                LT ->
                    balance nColor nKey nValue (insertHelp toComparable key value nLeft) nRight

                EQ ->
                    RBNode_elm_builtin nColor nKey value nLeft nRight

                GT ->
                    balance nColor nKey nValue nLeft (insertHelp toComparable key value nRight)
...
no changes are made.
-}
remove : (k -> comparable) -> k -> Dict k v -> Dict k v
remove toComparable key dict =
    -- Root node is always Black
    case removeHelp toComparable key dict of
        RBNode_elm_builtin Red k v l r ->
            RBNode_elm_builtin Black k v l r
...
up again.
-}
removeHelp : (k -> comparable) -> k -> Dict k v -> Dict k v
removeHelp toComparable targetKey dict =
    case dict of
        RBEmpty_elm_builtin ->
            RBEmpty_elm_builtin

        RBNode_elm_builtin color key value left right ->
            if toComparable targetKey < toComparable key then
                case left of
                    RBNode_elm_builtin Black _ _ lLeft _ ->
                        case lLeft of
                            RBNode_elm_builtin Red _ _ _ _ ->
                                RBNode_elm_builtin color key value (removeHelp toComparable targetKey left) right

                            _ ->
                                case moveRedLeft dict of
                                    RBNode_elm_builtin nColor nKey nValue nLeft nRight ->
                                        balance nColor nKey nValue (removeHelp toComparable targetKey nLeft) nRight
                ...
                removeHelpEQGT toComparable targetKey (removeHelpPrepEQGT targetKey dict color key value left right)
...
-}
removeHelpEQGT : (k -> comparable) -> k -> Dict k v -> Dict k v
removeHelpEQGT toComparable targetKey dict =
    case dict of
        RBNode_elm_builtin color key value left right ->
            if targetKey == key then
                ...
                balance color key value left (removeHelp toComparable targetKey right)
...
-}
update : (k -> comparable) -> k -> (Maybe v -> Maybe v) -> Dict k v -> Dict k v
update toComparable targetKey alter dictionary =
    case alter (get toComparable targetKey dictionary) of
        Just value ->
            insert toComparable targetKey value dictionary

        Nothing ->
            remove toComparable targetKey dictionary
...
-}
union : (k -> comparable) -> Dict k v -> Dict k v -> Dict k v
union toComparable t1 t2 =
    foldl (insert toComparable) t2 t1


{-| Keep a key-value pair when its key appears in the second dictionary.
Preference is given to values in the first dictionary.
-}
intersect : (k -> comparable) -> Dict k v -> Dict k v -> Dict k v
intersect toComparable t1 t2 =
    filter toComparable (\k _ -> member toComparable k t2) t1


{-| Keep a key-value pair when its key does not appear in the second dictionary.
-}
diff : (k -> comparable) -> Dict k a -> Dict k b -> Dict k a
diff toComparable t1 t2 =
    foldl (\k v t -> remove toComparable k t) t1 t2
...
merge :
    ...
    -> result
    -> result
merge toComparable leftStep bothStep rightStep leftDict rightDict initialResult =
    let
        stepState rKey rValue ( list, result ) =
            case list of
                ...
                ( lKey, lValue ) :: rest ->
                    if toComparable lKey < toComparable rKey then
                        stepState rKey rValue ( rest, leftStep lKey lValue result )

                    else if toComparable lKey > toComparable rKey then
                        ( list, rightStep rKey rValue result )
...
-}
filter : (k -> comparable) -> (k -> v -> Bool) -> Dict k v -> Dict k v
filter toComparable isGood dict =
    foldl
        (\k v d ->
            if isGood k v then
                insert toComparable k v d
...
-}
partition : (k -> comparable) -> (k -> v -> Bool) -> Dict k v -> ( Dict k v, Dict k v )
partition toComparable isGood dict =
    let
        add key value ( t1, t2 ) =
            if isGood key value then
                ( insert toComparable key value t1, t2 )

            else
                ( t1, insert toComparable key value t2 )
    in
...
{-| -}
first : Dict k v -> Maybe ( k, v )
first a =
    case a of
        RBNode_elm_builtin _ k v left _ ->
            case first left of
                Just b ->
                    Just b
...
{-| -}
last : Dict k v -> Maybe ( k, v )
last a =
    case a of
        RBNode_elm_builtin _ k v _ right ->
            case last right of
                Just b ->
                    Just b
